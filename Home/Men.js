...
const Men = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            const q = query(collection(db, "Items"), where("gender", "==", "Men"), where("status", "==", "Approved"));
    ...
    useEffect(() => {
        const MenCategories = list
            .filter(item => item.gender === 'Men')
            .map(item => item.category);
    
        const uniqueCategories = [...new Set(MenCategories)];
    ...
    return(
        <ScrollView style={styles.scroll} 
            ...
            <Text style = {styles.gender}>Men</Text>
            {categories.map((item, key) => (
                <TouchableOpacity
                    key = {key} 
                    style = {styles.button} 
                    onPress = { () => navigation.navigate('Category', {title: item, type: 'Men'})}>
